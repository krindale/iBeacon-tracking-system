...
## 📱 모바일 레이아웃 최적화 계획 (2026-01-08)

iBeacon 트래킹 관리자 대시보드 및 히스토리 페이지의 모바일 UX 개선 계획입니다.

### 대시보드 (Dashboard)
- **사용자 상태 테이블**: 모바일(`sm` 이하)에서 카드 리스트 형태로 전환하여 가로 스크롤 없이 확인 가능하도록 수정
- **내비게이션 개선**: 테이블 행(Row)이나 카드 전체를 클릭하면 히스토리 페이지로 이동하도록 개선 (작은 버튼 클릭의 불편함 해소)
- **헤더 액션**: 터치 친화적인 버튼 크기 및 배치 조정

### 히스토리 페이지 (History)
- **API 로그 사이드바**: 모바일에서 전체 화면 드로어(Drawer)로 변경하여 가시성 확보
- **날짜 선택기**: 터치 타켓 크기 확대 및 좁은 화면 대응 레이아웃 최적화
- **타임라인**: 좁은 기기에서 마커 및 텍스트 간격 조정
